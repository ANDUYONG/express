...
const helper = {
    createServer: (req: MyRequest, res: http.ServerResponse) => {
        ...
        if (matchedRoute) {
            ...
            // response helper적용
            myRes.status(200).json({ params: req.params, query: req.query }); // 상태 코드 설정
        } else {
            // response helper적용
...
const server = http.createServer(helper.createServer);
